...
public class ToggleCommand extends SubCommand {
    ...
    public ToggleCommand(AutoSellCommand autoSellCommand) {
    ...
    public void execute(CommandSender sender, String[] args) {
        if(sender instanceof Player) {
            ...
            if(autoSellCommand.getPlugin().getAutoSellManager().containsPlayer(player.getUniqueId())) {
                ...
                if(sellProfile == null) {
                    autoSellCommand.getPlugin().getAutoSellManager().addPlayer(player);
                    autoSellCommand.getPlugin().getUtils().sendMessage(sender, autoSellCommand.getMessagesConfig(), "ACTIVATED-AUTO-SELL", null);
                    return;
                }

                autoSellCommand.getPlugin().getAutoSellManager().removePlayer(sellProfile);
            } else {
                autoSellCommand.getPlugin().getAutoSellManager().addPlayer(player);
                autoSellCommand.getPlugin().getUtils().sendMessage(sender, autoSellCommand.getMessagesConfig(), "ACTIVATED-AUTO-SELL", null);
            }
